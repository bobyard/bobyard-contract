module bob::internal {
    ...
    public fun change_listing<T, ITEM: key + store>(
        marketplace: &mut Market<T>,
        list_id: ID,
        ask: u64,
        expire_time: u64,
    ...
    public fun delist<T, ITEM: key + store>(
        marketplace: &mut Market<T>,
        list_id: ID,
        ctx: &mut TxContext
    ) {
    ...
    public fun buy<T, ITEM: key + store>(
        marketplace: &mut Market<T>,
        item_id: ID,
        paid: Coin<T>,
        clock: &Clock,
        ...
        assert!(is_last(marketplace), ENotLastVersion);
        core::sweep<T, ITEM>(marketplace, item_id, &mut paid, clock, ctx);
    ...
    public fun accept_offer<T, BuyItem: key+store>(
        marketplace: &mut Market<T>,
        list_id: ID,
        offer_id: ID,
        clock: &Clock,
